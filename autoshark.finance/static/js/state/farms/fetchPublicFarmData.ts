...
function getABI(farm: Farm, tempMasterChef: any) {
  if (farm.masterChef === getOceanMasterChefAddress()) {
    return masterchefABI
  }
  return tempMasterChef.map((item) => (item.name === 'cakePerBlock' ? { ...item, name: farm.masterChefOutput } : item))
}
...
const fetchFarm = async (farm: Farm): Promise<PublicFarmData> => {
  ...
  const tempMasterChef = [...cakeMasterchefABI]

  // Only make masterchef calls if farm has pid
  const [info, totalAllocPoint] =
    pid || pid === 0
      ? await multicall(getABI(farm, tempMasterChef), [
          {
